...
export const backToTriage = (order) => {
  ...
  if(r) {
    ...
    axios.put(`${url}/${order._id}`, { triagecomplete: null, duedate: null, sameasChildren: newArray })
...
export const backToDesign = (order) => {
  ...
  if(r) {
    let newArray = order.sameasChildren
    if (order.sameasChildren.length > 0) {
      newArray.forEach((x) => { x.designcomplete = null })
    }
    axios.put(`${url}/${order._id}`, { designcomplete: null, sameasChildren: newArray })
    order.sameasChildren.forEach(child => {
